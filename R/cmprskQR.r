...
function(ftime, fstatus, X, 
	...
	ret <- crrQR.int(ftime, fstatus, X, tau.L=tau.range[1], tau.U=tau.range[2], tau.step=tau.step, outcome=failcode, cencode=cencode, rq.method=rq.method, orig.num=n, variance=variance)
		
	return(ret)
}
	

crrQR.int <- function(ftime, fstatus, X, tau.L, tau.U, tau.step, outcome=1, cencode=0, noconst=FALSE, variance = TRUE, offset=0, max.lmt=10^8, rq.method="br", orig.num=nrow(X), retry=TRUE, ...) {
	...
	cens <- (sum(FT0)>0)
  	...
  	if(cens) {
		...
		csurv.indx <- unlist(lapply(ftime, function(et) length(which(et>csurv.fit$time | (abs(et-csurv.fit$time)<smallest)))))
	...
	goon <- TRUE
  	while(goon) {
    	...
    	est.beta.obj <- try(switch(rq.method, fn = rq.fit.fnb(pseudo.cvt, pseudo.resp, tau = curr.tau, ...), 
				...
				br = rq.fit.br(pseudo.cvt, pseudo.resp, tau = curr.tau, ci=F, ...),
		...
		if(inherits(est.beta.obj, "try-error")) {
			if(retry) {
				...
				est.beta.obj <- rq.fit.br(pseudo.cvt, pseudo.resp, tau = curr.tau, ci=F, ...)
			} else {
	    ...
	    ind.conv <- as.numeric(abs(est.beta.obj$residuals[num+1])>1)*as.numeric(abs(est.beta.obj$residuals[num+2])>1)
    	...
    	if(ind.conv==1) {
			...
			if(variance) {
				helper <- as.numeric((ftime)<X%*%as.vector(est.beta))*FT1.csurv.x
	  			...
	  			sigma.sqrt <- (var.dcmp$evector%*%diag(sqrt(var.dcmp$evalues),length(var.dcmp$evalues),length(var.dcmp$evalues))%*%solve(var.dcmp$evector))  #eigen decomposition
				...
				est.beta.var <- NULL
      			...
      			for(k in 1:cvt.length) {
        			...
        			est.beta.var.obj <- try(switch(rq.method, fn = rq.fit.fnb(pseudo.cvt.var, pseudo.resp, tau = curr.tau, ...), 
						...
						br = rq.fit.br(pseudo.cvt.var, pseudo.resp, tau = curr.tau, ci=F, ...),
					...
					if(inherits(est.beta.var.obj, "try-error")) {
						if(retry) {
							cat('retry with method br ')
							est.beta.var.obj <- rq.fit.br(pseudo.cvt, pseudo.resp, tau = curr.tau, ci=F, ...)
						} else {
							stop('estimation not successful, try different method')
						}
					}
				
        			ind.conv.var<-c(ind.conv.var, as.numeric(abs(est.beta.var.obj$residuals[num+1])>1)*as.numeric(abs(est.beta.var.obj$residuals[num+2])>1))
...
function(object, conf.int = 0.95, digits = max(options()$digits - 5, 2), ...) {
	...
	ci.val <- qnorm((1-conf.int)/2)
	
	CI.lb.seq <- object$beta.seq-ci.val*sqrt(object$var.seq)
	CI.ub.seq <- object$beta.seq+ci.val*sqrt(object$var.seq)
	...
	for(k in 1:(object$cvt.length)) {
		...
		infs <- do.call("cbind", lapply(infs, function(x) x[,k,drop=F]))
		inf.ave <- cbind(inf.ave, apply(infs*tau.step, 1, sum)/(tau.U-tau.L)) 
		est.ave <- c(est.ave, sum(object$beta.seq[1: (L.tau.seq-1), k, drop=F]*tau.step)/(tau.U-tau.L))
		diff <- infs-matrix(rep.int(inf.ave[,k],ncol(infs)), ncol=(ncol(infs)), byrow=F)
		inf.cnst.test.w1 <- cbind(inf.cnst.test.w1, apply(t(t(diff)*tau.w1)*tau.step, 1, sum)/sum(tau.w1*tau.step)) 
		cnst.test.w1 <- c(cnst.test.w1, sum((object$beta.seq[1: (L.tau.seq-1), k, drop=F]-est.ave[k])*tau.w1*tau.step)/sum(tau.w1*tau.step))
	}
...
# add rearrangement=TRUE for using the rearrangement method to ensure increasing CiC predictions (cf. Chernozhukov and Fernández-Val and Galichon, Quantile and probability curves without crossing, Econometrica 78, 1093--1125)
function(object, x, ...) {
	rearrangement <- FALSE
